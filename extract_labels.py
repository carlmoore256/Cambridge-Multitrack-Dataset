...
def create_label_map(data_path, kw_path, c_thresh):

  kw_filter = FilterStems(file_utils.load_keywords(kw_path), c_thresh)
  ...
  for root, dirs, files in os.walk(data_path):
    ...
    if '__MACOSX' not in session_name:
      ...
      if len(matched_kw) > 0:
        print(f'{len(matched_kw)} total files matching keywords in {session_name}')
  return dir_map
